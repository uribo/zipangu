...
convert_prefecture_to_kanji <- function(x){
  ...
  for(i in 1:length(x)){
    if(any(str_detect(prefecture_list[[5]], x[i]))){
      x[i] <- prefecture_list[[1]][str_detect(prefecture_list[[5]], x[i])] #Convert japanese to roman
    } else if(any(str_detect(prefecture_list[[6]], x[i]))){
      x[i] <- prefecture_list[[2]][str_detect(prefecture_list[[6]], x[i])] #Convert japanese to roman
    } else {
...
convert_prefecture_to_roman <- function(x){
  ...
  for(i in 1:length(x)){
    if(any(str_detect(prefecture_list[[1]], x[i]))){
      x[i] <- prefecture_list[[5]][str_detect(prefecture_list[[1]], x[i])] #Convert japanese to roman
    } else if(any(str_detect(prefecture_list[[2]], x[i]))){
      x[i] <- prefecture_list[[6]][str_detect(prefecture_list[[2]], x[i])] #Convert japanese to roman
    } else {
      stop("\u5b58\u5728\u3057\u306a\u3044\u5730\u57df\u304c\u542b\u307e\u308c\u3066\u3044\u307e\u3059") #check;is Japan
    }
  }
