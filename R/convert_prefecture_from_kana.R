...
convert_prefecture_from_kana <- function(x){
  ...
  for(i in 1:length(x)){
    if(any(str_detect(prefecture_list[[3]], x[i]))){
      x[i] <- prefecture_list[[1]][str_detect(prefecture_list[[3]], x[i])]
    } else if(any(str_detect(prefecture_list[[4]], x[i]))){
      x[i] <- prefecture_list[[2]][str_detect(prefecture_list[[4]], x[i])]
    } else {
